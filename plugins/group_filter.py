...
async def next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"🎬[𝖳𝗋™] [{get_size(file.file_size)}] ➳ {file.file_name}", callback_data=f'{pre}#{req}#{file.file_id}')] for file in files ]
        else:
            btn = [[InlineKeyboardButton(text=f"➳ {file.file_name}", callback_data=f'{pre}#{req}#{file.file_id}'),
                    InlineKeyboardButton(text=f"🎬[𝖳𝗋™] {get_size(file.file_size)}", callback_data=f'{pre}#{req}#{file.file_id}')] for file in files ] 
