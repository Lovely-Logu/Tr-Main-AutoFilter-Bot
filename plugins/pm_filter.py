...
async def pm_next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"🎬[𝖳𝗋™] [{get_size(file.file_size)}] ➳ {file.file_name}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}"))] for file in files ]
        ...
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"🎬[𝖳𝗋™] [{get_size(file.file_size)}] ➳ {file.file_name}", callback_data=f'pmfile#{file.file_id}')] for file in files ]
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if SHORT_URL and SHORT_API:          
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"🎬[𝖳𝗋™] [{get_size(file.file_size)}] ➳ {file.file_name}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=pre_{file.file_id}"))] for file in files ]
        ...
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"🎬[𝖳𝗋™] [{get_size(file.file_size)}] ➳ {file.file_name}", callback_data=f'{pre}#{file.file_id}')] for file in files ]
